    ...
    if(pDir) {
        while((ent = readdir(pDir)) != NULL) {
            ...
            if (!strncasecmp(idVendor, "05c6", 4) || 
                ...
                (gvid && !strncasecmp(gvid, idVendor, 4)))
                log_debug("Find device vid:%s pid %s\n", idVendor, idProduct);
            else
                continue;
            /* for GW_jibei, no nema port, so interface should be 2 */
            if (!strncasecmp(idVendor, "3763", 4))
                interface = 2;
